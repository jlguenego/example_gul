...
var JLGLayoutCtrl = require('./JLGLayoutCtrl.js');
		...
		link: function(scope, element, attrs, ctrl) {
			...
			scope.$watch(ctrl.name + '.isMobile', function() {
				...
				if (ctrl.isMobile) {
					...
					element.prepend('<jlg-bars ng-click="' + ctrl.name + '.toggleMenu()" id="jlg_menu_bars" class="fa fa-bars fa-3x" aria-hidden="true"></jlg-bars>');
			...
			scope.$watchGroup([ctrl.name + '.isMobile', ctrl.name + '.isMenu'], function() {
...
app.directive('jlgMenuItem', function(navigate) {
	return {
		require: '^jlgLayout',
